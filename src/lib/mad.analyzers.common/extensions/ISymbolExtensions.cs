...
namespace mad.analyzers.common.extensions
{
    public static class ISymbolExtensions
    {
            ...
            if (symbol is IPropertySymbol propertySymbol)
            {
                foreach (ISymbol member in propertySymbol.ContainingType.GetMembers())
                {
                    if (member is IFieldSymbol associated &&
                        ...
                        SymbolEqualityComparer.Default.Equals(associated.AssociatedSymbol, propertySymbol))
                    {
        ...
        {
            return member != null && SymbolEqualityComparer.Default.Equals(member.ContainingType, type) && member.MetadataName == name;
        }
        ...
        {
            var parametersOfType = parameters.GetParametersOfType(type);
        ...
        {
            return parameters.Where(p => SymbolEqualityComparer.Default.Equals(p.Type, type));
        }
        ...
        public static bool HasOverloadWithParameterOfType(this IEnumerable<IMethodSymbol> overloads, IMethodSymbol self, INamedTypeSymbol type, CancellationToken cancellationToken)
        {
            foreach (var overload in overloads)
            {
                ...
                if (SymbolEqualityComparer.Default.Equals(self, overload) == true)
                {
                    continue;
                }
        ...
        {
            var set = new HashSet<IParameterSymbol>(parameters, SymbolEqualityComparer.Default);
        ...
        public static bool ParametersAreSame(this IMethodSymbol method1, IMethodSymbol method2)
        {
            ...
            {
                if (!method1.Parameters[index].ParameterTypesAreSame(method2.Parameters[index]))
                {
                    return false;
                }
            }
        ...
        public static bool IsSymbolWithSpecialDiscardName([NotNullWhen(returnValue: true)] this ISymbol? symbol)
            => symbol?.Name.StartsWith("_", StringComparison.Ordinal) == true &&
               (symbol.Name.Length == 1 || uint.TryParse(symbol.Name.Substring(1), out _) || symbol.Name.All(n => n.Equals('_')));
