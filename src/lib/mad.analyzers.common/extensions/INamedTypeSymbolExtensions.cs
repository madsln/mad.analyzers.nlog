...
#pragma warning disable CS8600  // A possible null reference is assigned to a non-nullable reference type.
                current = current.BaseType;
#pragma warning restore CS8600  // A possible null reference is assigned to a non-nullable reference type.
            }
        }
        ...
        public static bool DerivesFromOrImplementsAnyConstructionOf(this INamedTypeSymbol type, INamedTypeSymbol parentType)
        {
            ...
            {
                if (SymbolEqualityComparer.Default.Equals(baseType, parentType))
                {
                    return true;
                }
            }

            if (type.OriginalDefinition.AllInterfaces.Any(baseInterface => SymbolEqualityComparer.Default.Equals(baseInterface.OriginalDefinition, parentType)))
            {
                return true;
            }
