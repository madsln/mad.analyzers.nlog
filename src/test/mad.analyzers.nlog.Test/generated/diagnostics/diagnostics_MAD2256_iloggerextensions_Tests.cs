...
[TestClass]
public class diagnostics_MAD2256_iloggerextensions_Tests
{
    [TestMethod]
    [DataRow("Trace")]
    [DataRow("Debug")]
    ...
    [DataRow("Error")]
    [DataRow("Fatal")]
    public async Task ILoggerExtensions_Log_JsonSerializerSerializePassedAsArgument_ShouldRaiseDiagnostic_MAD2256(string logLevel)
    {
        var template = @"
        using System;
        using NLog;
        ...
        public class ILoggerExtensions_Log_JsonSerializerSerializePassedAsArgument
        {
            ...
            {
                _logger.ConditionalDebug(""Processing: {Dto}"", {|#0:JsonSerializer.Serialize(dto)|});
            }
        }
        ";
    ...
    [DataRow("Error")]
    [DataRow("Fatal")]
    public async Task ILoggerExtensions_Log_ToStringOnComplexObjectPassedAsArgument_ShouldRaiseDiagnostic_MAD2256(string logLevel)
    {
        var template = @"
        using System;
        using NLog;
        ...
        public class ILoggerExtensions_Log_ToStringOnComplexObjectPassedAsArgument
        {
            ...
            public void ShouldRaiseDiagnostic_MAD2256(Dto dto)
            {
                _logger.ConditionalDebug(""Processing: {Dto}"", {|#0:dto.ToString()|});
            }
        }
        ";
        var test = template.Replace("%LOGLEVEL%", logLevel);
